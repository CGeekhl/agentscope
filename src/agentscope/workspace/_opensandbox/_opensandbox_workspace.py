...
# 仅 OpenSandbox 沙箱模式生效（本类 override _gateway_proxy_url 才返回
# 非 None）；Docker/E2B 等无 server-proxy 的沙箱不受影响。传输层故障自动
# fallback 回 shim。关闭（"0"/"false"/"no"/"off"）：gateway 维持 127.0.0.1
# 绑定，走原 exec_shell shim 通道，行为与官方一致。
...
class OpenSandboxWorkspace(SandboxedWorkspaceBase):
    ...
    def _fork_config_lines(self) -> list[str]:
        ...
        lines.append(
            ...
            "spawn 开销；传输层故障自动 fallback 回 shim。"
